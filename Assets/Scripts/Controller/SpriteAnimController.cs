...
namespace PlatformerMVC
{
    public class SpriteAnimController : IDisposable
    {
        ...
        private Dictionary<SpriteRenderer, Animation> _activeAnims = new Dictionary<SpriteRenderer, Animation>();
        ...
        {
            if (_activeAnims.TryGetValue(spriteRenderer, out var animation))
            {
                animation.Loop = loop;
            ...
            {
                _activeAnims.Add(spriteRenderer, new Animation()
                {
                    Track = track,
        ...
        {
            if (_activeAnims.ContainsKey(sprite))
            {
                _activeAnims.Remove(sprite);
            }
        }

        public void Update() { 
            foreach (var animation in _activeAnims)
            {
